...
function update_history!(history::History, estimator::Estimator, tol::Real, elapsed::Real)
    ...
    h[:mse]               = isempty(estimator) ? nothing : mse(estimator)
    h[:rmse]              = isempty(estimator) ? nothing : rmse(estimator)
    h[:mean]              = isempty(estimator) ? nothing : mean(estimator)
    h[:var]               = isempty(estimator) ? nothing : var(estimator)
    h[:varest]            = isempty(estimator) ? nothing : varest(estimator)
    h[:bias]              = isempty(estimator) ? nothing : bias(estimator)
    ...
    h[:α]                 = isempty(estimator) ? nothing : α(estimator)
    h[:β]                 = isempty(estimator) ? nothing : β(estimator)
    h[:γ]                 = isempty(estimator) ? nothing : γ(estimator)
    h[:nb_of_samples]     = copy(nb_of_samples(estimator)) 
    h[:qoi_with_max_var]  = isempty(estimator) ? nothing : qoi_with_max_var(estimator)
