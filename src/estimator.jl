...
**`max_index_set_param`** is the maximum index set size parameter that can be used in a simulation. Typically, a simulation will only be possible up to a finite index set size parameter, due to, for instance, memory constraints or prohibitively lare computational cost. For a multilevel simulation with [`ML`](@ref) this is the maximum number of levels. For a multi-index simulation with [`FT`](@ref), [`TD`](@ref), [`HC`](@ref) or [`ZC`](@ref), this is the maximum index set size parameter. For simulations with [`AD`](@ref) or [`U`](@ref), this is the maximum index set size to be used in the `max_search_space` (see below). Default is 9223372036854775807 (`typemax(Int64)`). When this value is exceeded, the simulation will stop with a bias (and, consequently, a root mean square error) larger than the requested accuracy.
