...
function push!(history::History, estimator::Estimator, tol::Real)
    ...
    h[:γ]                  = γ(estimator)
    h[:δ]                  = δ(estimator)
    h[:nb_of_orig_samples] = deepcopy(nb_of_orig_samples(estimator))
    h[:nb_of_samples]      = deepcopy(nb_of_samples(estimator))
    h[:cost_model]         = apply_f(estimator, cost_model)
    h[:total_time]         = total_time(estimator)
