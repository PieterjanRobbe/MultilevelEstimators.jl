## lattices.jl : implementation of rank-1 lattice rules
#
...
julia> MultilevelEstimators.get_point(lattice_rule, 2)
2-element Array{Float64,1}:
 0.75
 0.5

julia> MultilevelEstimators.get_point(lattice_rule, 56) # returns random points beyond n
2-element Array{Float64,1}:
[...]
```
...
LatticeRule32(z::Vector{UInt32}, s::Integer, n::Integer) =
check_positive(s, "LatticeRule32", "number of dimensions s") &&
check_ordered(s, length(z)+1, "LatticeRule32", "number of dimensions s", "or equal to the length of the generating vector z") &&
check_ordered(n, 2^32, "LatticeRule32", "maximum number of points n", "or equal to 2^32, consider implementing a LatticeRule64 type") &&
...
julia> MultilevelEstimators.get_point(lattice_rule, 123)
...
"""
LatticeRule32(s::Int) = check_ordered(s, 3601, "LatticeRule32", "number of dimensions s", "or equal to 3600, please supply your own generating vector z to proceed") && LatticeRule32(joinpath(@__DIR__(), "generating_vectors", "K_3600_32.txt"), s)
