...
table_hline() = println(string("+", repeat(string(repeat("-", n()), "+"), 5)))
...
print_nb_of_samples(estimator::Estimator{<:AbstractIndexSet, <:QMC}, index::Index, b::Integer) = string(nb_of_shifts(estimator), " x ", b)
...
function print_optimal_nb_of_samples(samples::Dict)
    for index in sort(collect(keys(samples)))
        index_str = string(index)
        str = string("| ",index_str(), spaces(n()-2-length(index_str)))
        str = string(str, samples[index])
        println(str)
    end
end

print_optimal_nb_of_samples(samples::Integer) = println(string("| 0", spaces(n()-3), samples))
