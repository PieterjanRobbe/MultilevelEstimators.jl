...
function _run(estimator::MultiLevelQuasiMonteCarloEstimator, ϵ::T where {T<:Real})
    ...
    while !converged 
        ...
        while varest(estimator) > θ*ϵ^2 # doubling algorithm
            ...
            max_var_level = (indmax([varest(estimator,tau) for tau in keys(estimator)])-1,) # convert to level

            # increase the number of samples already taken
            if estimator.sample_multiplication_factor == 2 # (round to nearest power of two)
                n_opt = nextpow2(estimator.nsamples[max_var_level]+1) # + 1 to increase amount
            elseif estimator.sample_multiplication_factor <= 1
                n_opt = estimator.nsamples[max_var_level] + 1 # add 1 sample
            else
                n_opt = ceil(Int,estimator.nsamples[max_var_level]*estimator.sample_multiplication_factor)
            end

            # print optimal number of samples
            estimator.verbose && print_number_of_samples(estimator,Dict(max_var_level=>n_opt))

            # take additional samples
            n_due = n_opt - estimator.nsamples[max_var_level]
            n_due > 0 && sample!(estimator,max_var_level,n_due)
        end
