...
@testset "(lognormal) MIMC             " begin
	estimator = init_lognormal(HC(2), MC(),
							   qoi=Qoi2(),
							   cost_model=index->4*2^sum(index),
							   max_splitting=0.9,
							   nb_of_coarse_dofs=16,
							   folder=my_temp_folder
							   )
	run(estimator, 1e-2)
	estimator = init_lognormal(TD(2), MC(),
							   nb_of_coarse_dofs=8,
							   grf_generator=KarhunenLoeve(100),
							   covariance_function=Matern(1,2),
							   max_index_set_param=6,
							   folder=my_temp_folder
							   )
	run(estimator, 5e-3)
	estimator = init_lognormal(FT(2), MC(),
							   qoi=Qoi3(),
							   nb_of_qoi=16, 
							   nb_of_coarse_dofs=32,
							   max_index_set_param=3,
